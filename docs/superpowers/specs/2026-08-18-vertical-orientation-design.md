...
- `scripts/projectile.gd` — add `_progress` (starts at the launching rival's `_progress`, decreases at `projectile_speed`), rendered via `get_track_y`; hit-check gains a hop-height condition (a projectile arriving while the player is airborne above `BASELINE_Y` should miss, mirroring how it already only threatens a grounded player today)
  - **Resolved 2026-08-19 (final branch review): tested, hop does not clear projectiles** (a hop moves the player's capsule up, toward a descending projectile, so contact happens earlier, not later — there is no hop timing that avoids a hit). No hop-height condition was added to `projectile.gd`'s hit-check; design intent is confirmed as **bark-only avoidance** for projectiles (obstacles/treats remain genuinely hop-dodgeable via the progress-crossing + `is_hop_clearing()` pattern). See `scripts/projectile.gd`'s header comment for the current, accurate behavior.
...
- Projectile-vs-hop-height hit detection is new logic (the horizontal game never needed to check "is the player currently elevated" because hop height and forward position were on different, unrelated axes there; here they still are in spirit — the player's hop offset and every other entity's progress-based render are logically separate — but it's worth an explicit test that a well-timed hop actually avoids a projectile arriving at that instant, not just a static obstacle).
  - **Resolved 2026-08-19 (final branch review): tested, hop does not clear projectiles** (moves player toward descending projectile, making contact earlier rather than avoiding it). Design decision: projectiles are avoided by barking only, not hopping — no hop-height condition was added to the hit-check. This risk is closed; see `scripts/projectile.gd`'s header comment and the corresponding note above.
